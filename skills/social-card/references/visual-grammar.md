...
| Image overlay + subject safety rules | Selection-first stance, localized image-toned tint fallback, subject-map MDX comments, crop guards via `image.position` table | `references/image-overlay.md` |
| Screenshot treatment utility (`.swiss-frame-shot` + `.swiss-device-browser` + `.swiss-device-phone`) | CSS-only stages (paper / paper-2 / grey-1 / grid / dot / ink), corner / shadow / inset modifiers, two device chrome wrappers | `theme/patterns/swiss.css` + `references/screenshot-treatment.md` |

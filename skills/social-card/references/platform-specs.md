...
Pixel sizes, safe areas, and density expectations per supported platform. v1 ships one geometry that works across Instagram, Facebook, and Threads.
...
| Body floor | 30 px (`--social-card-body`) |
| Caption / micro floor | 22 px (`--social-card-micro`) |
| Density floor | Filled bands ≥ 70 % of canvas height (≥ 945 px of 1350) |
...
- **Instagram**: 4:5 is the largest portrait the feed shows without crop. 1:1 is also accepted but uses less vertical real estate.
- **Facebook**: portrait posts display at full height in the feed; 4:5 matches IG so the same export works in both places.
...
Mixed-ratio carousels (e.g. 4:5 hero + 1:1 detail) need per-frame geometry which lives on the OpenPress framework side — see `NOTES.md` GAP-3.
...
Do not silently extend the `social-4x5` geometry to other aspects — declare each one explicitly.
